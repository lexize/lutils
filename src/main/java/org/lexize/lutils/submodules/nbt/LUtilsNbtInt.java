...
public class LUtilsNbtInt extends LUtilsNbtValue<Integer> implements LUtilsNbtValue.NbtGettable<Integer>, LUtilsNbtValue.NbtSettable<Integer> {
    ...
    public NbtReturnValue getValue(LUtilsInputStream stream) {
        try {
            ...
            int value = ((valueBytes[0] & 0xFF) << 24) +
                    ...
                    ((valueBytes[2] & 0xFF) << 8 ) +
                    ((valueBytes[3] & 0xFF)      );
            return new NbtReturnValue(new String(nameBytes), new LUtilsNbtInt(value), 0);
        } catch (IOException e) {
    ...
    public NbtReturnValue getPureValue(byte[] bytes, int offset) {
        ...
        int value = ((valueBytes[0] & 0xFF) << 24) +
                ...
                ((valueBytes[2] & 0xFF) << 8 ) +
                ((valueBytes[3] & 0xFF)      );
        return new NbtReturnValue(null, new LUtilsNbtInt(value), valueOffset+4);
    }
    ...
    public NbtReturnValue getPureValue(LUtilsInputStream stream) {
        try {
            ...
            int value = ((valueBytes[0] & 0xFF) << 24) +
                    ...
                    ((valueBytes[2] & 0xFF) << 8 ) +
                    ((valueBytes[3] & 0xFF)      );
            return new NbtReturnValue(null, new LUtilsNbtInt(value), 0);
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
    }
